...
const getZoneName = (name: string | { es?: string; en?: string; fr?: string }): string => {
  ...
  if (name && typeof name === 'object') return name.es || name.en || name.fr || 'Zona'
  return 'Zona'
}

interface SetAnalyticsData {
  propertySet: {
    id: string
    name: string
  ...
  insights: {
    mostViewedZone: { name: string | { es?: string; en?: string; fr?: string }; views: number; propertyName?: string } | null
    leastViewedZone: { name: string | { es?: string; en?: string; fr?: string }; views: number; propertyName?: string } | null
    totalZoneViews: number
  }
  dailyStats: Array<{ date: string; views: number; uniqueVisitors: number; whatsappClicks: number }>
  recentEvaluations: Array<{ id: string; rating: number; comment: string | null; createdAt: string }>
...
function BarChart({
  ...
  if (data.length === 0) {
    return <div className="flex items-center justify-center h-48 text-gray-400">{noDataText}</div>
  }
          ...
          <div key={index} className="flex-1 flex flex-col items-center group">
            <div className="relative w-full flex justify-center mb-1">
              <motion.div
                ...
                style={{ backgroundColor: color, opacity: 0.8, minHeight: '4px' }}
            ...
            <span className="text-[10px] text-gray-500 truncate max-w-full">{item[labelKey]}</span>
          </div>
        )
      })}
    </div>
...
export default function PropertySetAnalyticsPage({ params }: { params: Promise<{ id: string }> }) {
  const [id, setId] = useState<string>('')
  const [loading, setLoading] = useState(true)
  const [error, setError] = useState<string | null>(null)
  const [data, setData] = useState<SetAnalyticsData | null>(null)
  ...
  useEffect(() => {
    ...
    const fetchAnalytics = async () => {
      try {
        setLoading(true)
        setError(null)
        const response = await fetch(`/api/property-sets/${id}/analytics/detailed`, {
          credentials: 'include'
        })
  ...
  const maxPropertyViews = Math.max(...data.propertyBreakdown.map(p => p.totalViews), 1)
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            <div className="flex items-center gap-4 min-w-0">
              <Link href={`/properties/groups/${id}`}>
                <button className="p-2 hover:bg-gray-100 rounded-full transition-colors">
                  <ArrowLeft className="w-5 h-5 text-gray-600" />
                </button>
              </Link>
              <div className="min-w-0">
                <h1 className="text-lg font-semibold text-gray-900 truncate">Analíticas del conjunto</h1>
                <p className="text-sm text-gray-500 truncate">
                  {data.propertySet.name} · {data.propertySet.propertiesCount} apartamentos
                </p>
              </div>
            </div>
        ...
        {activeTab === 'overview' && (
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <MetricCard
                title="Apartamentos"
                value={data.propertySet.propertiesCount}
                subtitle={`${data.propertySet.publishedCount} publicados`}
                icon={Building2}
                color="blue"
              />
              <MetricCard
            ...
            {data.propertyBreakdown.length > 0 && (
              <Card className="p-6">
                <div className="flex items-center justify-between mb-6">
                  <h3 className="font-semibold text-gray-900">Desglose por apartamento</h3>
                  <span className="text-sm text-gray-500">{data.propertyBreakdown.length} apartamentos</span>
                </div>
                <div className="space-y-4">
                  {data.propertyBreakdown.map((p, index) => (
                    <Link key={p.id} href={`/properties/${p.id}/analytics`} className="block hover:bg-gray-50 -mx-2 px-2 py-1 rounded transition-colors">
                      <HorizontalBar
                        label={p.name}
                        sublabel={`${p.propertyViews} perfil · ${p.zoneViews} zonas`}
                        value={p.totalViews}
                        maxValue={maxPropertyViews}
                        color={index === 0 ? '#6366f1' : '#a78bfa'}
                        rank={index + 1}
                      />
                    </Link>
                  ))}
              ...
              <Card className="p-6">
                ...
                <BarChart
                  data={Array.from({ length: 24 }, (_, i) => {
                    const hourData = data.viewsByHour.find(h => h.hour === i)
                    return { label: `${String(i).padStart(2, '0')}h`, value: hourData?.count || 0 }
                  })}
                  labelKey="label"
                  valueKey="value"
                  color="#8b5cf6"
                  ...
                  <div className="bg-green-50 border border-green-100 rounded-xl p-5">
                    <div className="flex items-center gap-3 mb-2">
                      ...
                      <div>
                        ...
                        <p className="text-lg font-bold text-green-900">{getZoneName(data.insights.mostViewedZone.name)}</p>
                        {data.insights.mostViewedZone.propertyName && (
                          <p className="text-xs text-green-700">{data.insights.mostViewedZone.propertyName}</p>
                        )}
                      </div>
                    </div>
                  ...
                  <div className="bg-orange-50 border border-orange-100 rounded-xl p-5">
                    <div className="flex items-center gap-3 mb-2">
                      ...
                      <div>
                        ...
                        <p className="text-lg font-bold text-orange-900">{getZoneName(data.insights.leastViewedZone.name)}</p>
                        {data.insights.leastViewedZone.propertyName && (
                          <p className="text-xs text-orange-700">{data.insights.leastViewedZone.propertyName}</p>
                        )}
                      </div>
                    </div>
          ...
          <div className="space-y-6">
            <Card className="p-6">
              ...
              {data.zones.length === 0 ? (
                ...
                <div className="space-y-4">
                  {data.zones.slice(0, 30).map((zone, index) => (
                    <HorizontalBar
                      key={zone.id}
                      label={getZoneName(zone.name)}
                      sublabel={zone.propertyName}
                  ...
                  {data.zoneReviews.map((review) => (
                    <div key={review.id} className="bg-gray-50 rounded-lg p-4 border border-gray-100">
                      <div className="flex items-start justify-between mb-2">
                        <div>
                          <p className="font-medium text-gray-900 text-sm">{getZoneName(review.zoneName)}</p>
                          <p className="text-xs text-gray-500">{formatTimeAgo(review.createdAt)}</p>
                        </div>
                        <StarRating rating={review.overallRating} />
                      </div>
                      {review.feedback && (
                        <p className="text-sm text-gray-600 mb-3 italic">"{review.feedback}"</p>
                      )}
                      <div className="grid grid-cols-2 gap-2 text-xs">
                        <div className="flex justify-between">
                          <span className="text-gray-500">{t('analytics.reviewsTab.labels.clarity')}</span>
                          <span className="font-medium">{review.clarity}/5</span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-gray-500">{t('analytics.reviewsTab.labels.completeness')}</span>
                          <span className="font-medium">{review.completeness}/5</span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-gray-500">{t('analytics.reviewsTab.labels.helpfulness')}</span>
                          <span className="font-medium">{review.helpfulness}/5</span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-gray-500">{t('analytics.reviewsTab.labels.upToDate')}</span>
                          <span className="font-medium">{review.upToDate}/5</span>
                        </div>
                      </div>
                    </div>
                  ))}
                  ...
                  {data.recentEvaluations.map((evaluation) => (
                    <div key={evaluation.id} className="flex items-start gap-4 p-4 bg-gray-50 rounded-lg">
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-xs text-gray-500 mt-1">{formatTimeAgo(evaluation.createdAt)}</p>
                      </div>
                    </div>
                  ))}
                </div>
                  ...
                  {data.zoneComments.map((comment) => (
                    <div key={comment.id} className="p-4 bg-gray-50 rounded-lg">
                      <div className="flex items-center justify-between mb-2">
                        <span className="text-sm font-medium text-gray-900">{getZoneName(comment.zoneName)}</span>
