...
export async function POST(request: NextRequest) {
  try {
    ...
    const results = {
      ...
      _debug: [] as Array<{ guest: string; id?: string; error?: string }>
    }
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      try {
        ...
        // Check for duplicate by confirmation code OR by guest+dates (same property)
        if (skipDuplicates) {
          const propertyFilter = billingUnitId ? { billingUnitId } : { billingConfigId: billingConfigId! }

          // First check by confirmation code (if it's a real code, not auto-generated)
          const isAutoGenerated = parsed.confirmationCode.startsWith('UNI-')
          if (!isAutoGenerated && parsed.confirmationCode) {
            const existingByCode = await prisma.reservation.findFirst({
              where: { userId, confirmationCode: parsed.confirmationCode, ...propertyFilter }
            })
            if (existingByCode) {
              results.skippedCount++
              continue
          ...
          // Always check by guest name + check-in + check-out (catches re-imports)
          if (parsed.guestName && parsed.checkIn && parsed.checkOut) {
            const existingByData = await prisma.reservation.findFirst({
              where: {
                userId,
        ...
        const created = await prisma.reservation.create({ data: createData })

        // Verify the row actually persisted
        const verify = await prisma.reservation.findUnique({ where: { id: created.id } })
        results._debug.push({
          guest: createData.guestName,
          id: created.id,
    ...
    // Verify: count all CSV_UNIVERSAL reservations for this user
    const verifyCount = await prisma.reservation.count({
    ...
    return NextResponse.json({
      ...
      results,
      _debug: {
        createdIds: results._debug,
        verifyCSVUniversalCount: verifyCount,
