 ...
 *
 * Query params:
 * - module: 'MANUALES' | 'GESTION' | undefined (si no se especifica, devuelve ambos)
 */
export async function GET(request: NextRequest) {
  try {
    ...
    const moduleParam = searchParams.get('module')?.toUpperCase() as ModuleCode | undefined

    // Si se especifica un módulo, devolver solo ese
    if (moduleParam && (moduleParam === 'MANUALES' || moduleParam === 'GESTION')) {
      if (moduleParam === 'MANUALES') {
        const access = await moduleLimitsService.getManualesAccess(userId)
        return NextResponse.json({
          success: true,
          module: MODULES[moduleParam],
          access
        })
      } else {
