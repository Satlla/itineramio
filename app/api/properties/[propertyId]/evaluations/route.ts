...
export async function GET(
  ...
  try {
    ...
    const transformedReviews = reviews.map(review => ({
      ...
      zone: review.zone ? {
        id: review.zone.id,
        name: typeof review.zone.name === 'string' ? review.zone.name : review.zone.name.es || 'Zona',
    ...
    const transformedZoneRatings = zoneRatings.map(rating => ({
      ...
      zone: {
        id: rating.zone.id,
        name: typeof rating.zone.name === 'string' ? rating.zone.name : rating.zone.name.es || 'Zona',
    ...
    const averageRating = totalEvaluations > 0 
      ? allEvaluations.reduce((sum, eval) => sum + eval.rating, 0) / totalEvaluations 
      : 0

    // Calculate rating distribution
    const ratingDistribution = { 5: 0, 4: 0, 3: 0, 2: 0, 1: 0 }
    allEvaluations.forEach(eval => {
      ratingDistribution[eval.rating as keyof typeof ratingDistribution]++
    })

    const publicEvaluations = allEvaluations.filter(eval => eval.isPublic).length
