...
export async function POST(
  ...
  try {
    ...
    const { planId, billingPeriod = 'MONTHLY', sendEmail: shouldSendEmail, markAsPaid } = await request.json()
    ...
    if (!['MONTHLY', 'SEMESTRAL', 'YEARLY'].includes(billingPeriod)) {
      return NextResponse.json({
        success: false,
    ...
    // Calculate end date and price based on billing period
    const getDaysForPeriod = (period: string): number => {
      switch (period) {
        case 'MONTHLY': return 30
        case 'SEMESTRAL': return 180 // 6 months
        case 'YEARLY': return 365 // 12 months
        default: return 30
      }
    }

    const getPriceForPeriod = (plan: any, period: string): number => {
      switch (period) {
        case 'MONTHLY':
          return Number(plan.priceMonthly)
        case 'SEMESTRAL':
          return Number(plan.priceSemestral || plan.priceMonthly * 6)
        case 'YEARLY':
    ...
    const daysForPeriod = getDaysForPeriod(billingPeriod)
    const priceForPeriod = getPriceForPeriod(newPlan, billingPeriod)
    ...
    const newSubscription = await prisma.userSubscription.create({
      data: {
        ...
        status: 'active',
        startDate: new Date(),
        endDate: new Date(Date.now() + daysForPeriod * 24 * 60 * 60 * 1000),
        notes: `Plan cambiado por administrador - Período: ${periodLabel}${markAsPaid ? ' - Marcado como pagado' : ''}`
      }
    })
    ...
    if (markAsPaid) {
      ...
      await prisma.invoice.create({
        data: {
          ...
          discountAmount: 0,
          finalAmount: priceForPeriod,
          status: 'paid',
          paidDate: new Date(),
          dueDate: new Date(Date.now() + daysForPeriod * 24 * 60 * 60 * 1000),
          paymentMethod: 'manual',
          notes: `Plan cambiado manualmente por administrador - ${periodLabel}`
        }
      })
    }
              ...
              <div class="container">
                ...
                <div class="content">
                  ...
                  <div class="plan-box">
                    ...
                    <p><strong>Período de facturación:</strong> ${periodLabel}</p>
                    <p><strong>Precio:</strong> €${priceForPeriod.toFixed(2)} por ${periodLabel.toLowerCase()}</p>
