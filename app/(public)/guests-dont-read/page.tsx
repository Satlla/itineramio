...
import { X, Star, RefreshCw, ArrowRight, Key, Wifi, Car, ChefHat, ClipboardList, MessageCircle, PenLine, Rocket, Home, FileText, Utensils, PhoneCall, Moon } from 'lucide-react'
    ...
    <div className="min-h-screen bg-white text-[#111] overflow-x-hidden"
      ...
      <nav className="sticky top-0 left-0 right-0 z-50 bg-white border-b border-black/[0.08]"
        ...
        <div className="max-w-6xl mx-auto px-6 h-16 flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center gap-7">
            {[['Cómo funciona','#how'],['Por qué funciona','#why'],['Preguntas','#faq'],['Empieza','/register']].map(([l,h]) => (
              <a key={l} href={h} className="text-sm text-[#666] hover:text-[#111] transition-colors font-medium">{l}</a>
            ))}
          </div>
          <div className="flex items-center gap-3">
            <Link href="/login" className="text-sm text-[#666] hover:text-[#111] font-medium transition-colors px-3 py-2 rounded">Entrar</Link>
            <Link href="/register"
              className="hidden sm:inline-flex items-center gap-1.5 px-5 py-2.5 rounded-full text-sm font-semibold text-white transition-all"
              style={{ backgroundColor: '#7c3aed', boxShadow: '0 2px 12px rgba(124,58,237,0.3)' }}>
              Empieza gratis <ArrowRight className="w-3.5 h-3.5"/>
            </Link>
            <button
              className="md:hidden p-3 -mr-2 flex items-center justify-center min-w-[44px] min-h-[44px]"
              aria-label={mob ? 'Cerrar menú' : 'Abrir menú'}
              onClick={() => setMob(!mob)}>
        ...
        {mob && (
          <div className="md:hidden bg-white border-t border-black/5 px-6 py-4 flex flex-col gap-1">
            {[['Cómo funciona','#how'],['Por qué funciona','#why'],['Preguntas','#faq']].map(([l,h]) => (
              <a key={l} href={h} className="text-sm text-[#666] font-medium py-3 border-b border-black/[0.04]" onClick={() => setMob(false)}>{l}</a>
            ))}
            <div className="pt-3 flex flex-col gap-3">
              <Link href="/login" className="text-sm text-[#666] font-medium py-2" onClick={() => setMob(false)}>Entrar</Link>
              <Link href="/register"
                className="inline-flex items-center justify-center gap-1.5 px-5 py-3 rounded-full text-sm font-semibold text-white"
      ...
      <section className="pt-16 pb-4 px-6 bg-white">
        <div className="max-w-6xl mx-auto">
          ...
          <p className="text-[#555] text-[15px] sm:text-[17px] leading-relaxed mb-8 max-w-xl">
      ...
      <section className="pt-12 pb-20 px-6 bg-white">
        <div className="max-w-6xl mx-auto">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-16 items-center">
            <div className="h-[220px] sm:h-[300px] lg:h-[420px] rounded-2xl overflow-hidden">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-16 items-center">
            <div>
              <h2 className="font-bold text-[#111] leading-tight mb-6">
                <span className="block font-normal text-[#111]" style={{ fontSize: 'clamp(1rem, 2.5vw, 1.6rem)' }}>Y si el huésped llegara</span>
                <span className="block text-[#111]" style={{ fontSize: 'clamp(1.8rem, 5vw, 3.8rem)' }}>sabiendo cómo entrar?</span>
              </h2>
        ...
        <div className="max-w-6xl mx-auto">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {[
              ...
              <div key={i} className="rounded-2xl p-6 sm:p-7" style={{ ...gradientBorder }}>
                <h3 className="font-bold mb-3 leading-tight" style={{ ...gradientStyle, fontSize: 'clamp(1.1rem, 2.5vw, 1.75rem)' }}>{col.title}</h3>
      ...
      {/* ── TRES PASOS ── */}
      <section id="how" className="py-20 px-6" style={{ backgroundColor: '#fafafa' }}>
        <div className="max-w-5xl mx-auto text-center">
          <h2 className="font-bold text-[#111] leading-tight mb-12"
            style={{ fontSize: 'clamp(2rem, 4vw, 3rem)' }}>
            Tres pasos. Diez minutos.
          </h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 text-left mb-10">
            {[
              {
              ...
              <div key={i} className="bg-white rounded-2xl p-6 sm:p-8 border border-black/[0.06]"
                style={{ boxShadow: '0 2px 8px rgba(0,0,0,0.04)' }}>
                <div className="w-14 h-14 rounded-2xl flex items-center justify-center mb-5"
                  style={{ background: s.iconBg }}>
                  {s.icon}
                </div>
                <p className="text-[#ccc] font-bold leading-none mb-3" style={{ fontSize: 'clamp(1.6rem, 4vw, 2.5rem)' }}>{s.n}</p>
          ...
          <Link href="/register"
            className="inline-flex items-center gap-2 text-white px-7 sm:px-10 py-4 rounded-full font-semibold text-[15px]"
            style={{ backgroundColor: '#7c3aed' }}>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-16 items-center">
            ...
            <div>
              ...
              <div className="space-y-3">
                {[
                  { icon: <Moon className="w-4 h-4"/>, color: '#22c55e', text: 'Menos mensajes de WiFi a las 22:00' },
      ...
      <section className="py-20 px-6 bg-white">
        <div className="max-w-5xl mx-auto">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-16 items-center mb-12">
            ...
            <div className="h-[220px] sm:h-[300px] lg:h-[360px] rounded-2xl overflow-hidden">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 text-center">
            {[
              ...
              <div key={i} className="bg-white rounded-2xl p-6 sm:p-8 border border-black/[0.06]"
                style={{ boxShadow: '0 2px 8px rgba(0,0,0,0.04)' }}>
                <p className="font-bold mb-2" style={{ fontSize: 'clamp(1.6rem, 4vw, 2.2rem)', ...gradientStyle }}>{s.stat}</p>
      ...
      <section className="py-28 px-6 text-center"
        ...
        <div className="max-w-3xl mx-auto">
          ...
          <Link href="/register"
            className="inline-flex items-center gap-2 bg-white px-7 sm:px-10 py-4 rounded-full font-semibold text-[16px] text-[#7c3aed]">
            Prueba gratis 15 días
          </Link>
        </div>
      </section>
