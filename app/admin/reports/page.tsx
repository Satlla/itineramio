    ...
    <div className="p-3 sm:p-4 lg:p-6 space-y-4 sm:space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 sm:gap-4">
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center space-y-2 sm:space-y-0 sm:space-x-2 lg:space-x-3 w-full sm:w-auto">
          ...
          <button
            ...
            className="bg-gray-200 text-gray-700 px-3 py-1.5 sm:px-4 sm:py-2 rounded-lg hover:bg-gray-300 flex items-center justify-center text-xs sm:text-sm"
          >
            <Filter className="h-4 w-4 sm:h-5 sm:w-5 mr-2" />
            Filtros
            <ChevronDown className={`h-4 w-4 sm:h-5 sm:w-5 ml-1 transition-transform ${showAdvancedFilters ? 'rotate-180' : ''}`} />
          </button>
      ...
      {showAdvancedFilters && (
        <div className="bg-white rounded-lg border border-gray-200 p-3 sm:p-4 lg:p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
            ...
            <div>
              ...
              <select
                ...
                className="w-full px-3 py-1.5 sm:py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-red-500 focus:border-red-500 text-xs sm:text-sm"
              >
          ...
          {/* Filter Actions */}
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 sm:gap-0 mt-4 sm:mt-6 pt-3 sm:pt-4 border-t border-gray-200">
            <button
              onClick={() => setFilters({
                propertyId: '',
                zoneId: '',
      ...
      {reportData && (
        <>
          {/* Summary Metrics */}
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-3 sm:gap-4 lg:gap-6">
            <MetricCard
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-3 sm:gap-4 lg:gap-6">
                ...
                {reportData.topPerformers.zones.slice(0, 5).map((zone, index) => (
                  <div key={zone.id} className="flex items-center justify-between p-2 sm:p-3 bg-gray-50 rounded-lg">
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs sm:text-sm text-gray-600 truncate">{zone.property}</p>
                    </div>
                    <div className="flex items-center space-x-1 sm:space-x-2">
                      <span className="bg-blue-100 text-blue-800 px-1.5 py-0.5 sm:px-2 sm:py-1 rounded-full text-xs font-medium">
                        #{index + 1}
                      </span>
                      <span className="text-sm sm:text-base font-bold text-gray-900">{zone.views}</span>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className="bg-white rounded-lg border border-gray-200 p-3 sm:p-4 lg:p-6">
              <h3 className="text-base sm:text-lg font-semibold text-gray-900 mb-3 sm:mb-4 flex items-center">
                <PieChart className="h-4 w-4 sm:h-5 sm:w-5 mr-2 text-purple-500" />
                Métricas de Engagement
              </h3>
              <div className="space-y-2 sm:space-y-3 lg:space-y-4">
                <div className="flex justify-between items-center p-2 sm:p-3 bg-gray-50 rounded-lg">
                  <span className="text-xs sm:text-sm font-medium text-gray-700">Promedio vistas por zona</span>
                  <span className="text-sm sm:text-base font-bold text-gray-900">{reportData.engagement.avgViewsPerZone}</span>
                </div>
                <div className="flex justify-between items-center p-2 sm:p-3 bg-gray-50 rounded-lg">
                  <span className="text-xs sm:text-sm font-medium text-gray-700">Interacciones IA/día</span>
                  <span className="text-sm sm:text-base font-bold text-gray-900">{reportData.engagement.avgChatbotInteractionsPerDay}</span>
                </div>
                <div className="flex justify-between items-center p-2 sm:p-3 bg-gray-50 rounded-lg">
                  <span className="text-xs sm:text-sm font-medium text-gray-700">Hora más activa</span>
                  <span className="text-sm sm:text-base font-bold text-gray-900">{reportData.engagement.mostActiveTimeOfDay}</span>
                </div>
              </div>
            </div>
              ...
              <div className="space-y-2 sm:space-y-3">
                {reportData.engagement.popularLanguages.map((lang) => (
                  <div key={lang.language} className="flex items-center justify-between">
                    <span className="text-xs sm:text-sm font-medium text-gray-700">{lang.language}</span>
                    <div className="flex items-center space-x-2">
                      ...
                      <span className="text-xs sm:text-sm font-bold text-gray-900 min-w-[32px] text-right">{lang.percentage}%</span>
                    </div>
                  </div>
                ))}
              </div>
              ...
              <div className="space-y-2 sm:space-y-3">
                {reportData.topPerformers.properties.slice(0, 5).map((property, index) => (
                  <div key={property.id} className="flex items-center justify-between p-2 sm:p-3 bg-gray-50 rounded-lg">
                    <div className="flex-1 min-w-0">
                      <p className="text-sm sm:text-base font-medium text-gray-900 truncate">{property.name}</p>
                      <p className="text-xs sm:text-sm text-gray-600">{property.zonesCount} zonas</p>
                    </div>
                    <div className="flex items-center space-x-1 sm:space-x-2">
                      <Eye className="h-3 w-3 sm:h-4 sm:w-4 text-gray-400" />
                      <span className="text-sm sm:text-base font-bold text-gray-900">{property.totalViews}</span>
                    </div>
                  </div>
                ))}
              </div>
          ...
          <div className="bg-white rounded-lg border border-gray-200 p-3 sm:p-4 lg:p-6">
            <h3 className="text-base sm:text-lg font-semibold text-gray-900 mb-3 sm:mb-4 flex items-center">
              <Download className="h-4 w-4 sm:h-5 sm:w-5 mr-2 text-green-500" />
              Exportar Reporte
            </h3>
            ...
            <div className="flex flex-col sm:flex-row flex-wrap gap-2 sm:gap-3">
              <button
                ...
                className="bg-red-600 text-white px-3 py-1.5 sm:px-4 sm:py-2 rounded-lg hover:bg-red-700 flex items-center justify-center disabled:opacity-50 disabled:cursor-not-allowed text-xs sm:text-sm"
              >
                {isExporting ? <Loader2 className="h-4 w-4 sm:h-5 sm:w-5 mr-2 animate-spin" /> : <FileText className="h-4 w-4 sm:h-5 sm:w-5 mr-2" />}
                Exportar PDF
              </button>
              <button
                onClick={() => exportReport('excel')}
                disabled={isExporting}
                className="bg-green-600 text-white px-3 py-1.5 sm:px-4 sm:py-2 rounded-lg hover:bg-green-700 flex items-center justify-center disabled:opacity-50 disabled:cursor-not-allowed text-xs sm:text-sm"
              >
                {isExporting ? <Loader2 className="h-4 w-4 sm:h-5 sm:w-5 mr-2 animate-spin" /> : <BarChart3 className="h-4 w-4 sm:h-5 sm:w-5 mr-2" />}
                Exportar Excel
              </button>
              <button
                onClick={() => exportReport('csv')}
                disabled={isExporting}
                className="bg-blue-600 text-white px-3 py-1.5 sm:px-4 sm:py-2 rounded-lg hover:bg-blue-700 flex items-center justify-center disabled:opacity-50 disabled:cursor-not-allowed text-xs sm:text-sm"
              >
                {isExporting ? <Loader2 className="h-4 w-4 sm:h-5 sm:w-5 mr-2 animate-spin" /> : <Download className="h-4 w-4 sm:h-5 sm:w-5 mr-2" />}
                Exportar CSV
              </button>
            </div>
          </div>
