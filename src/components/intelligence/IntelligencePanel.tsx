...
import { getIntelligenceCompletion } from '@/types/intelligence'
import {
...
} from '@/config/intelligence-questions'
import type { QuestionDef, SectionDef } from '@/config/intelligence-questions'
...
export function IntelligencePanel({ propertyId, propertyName: propertyNameProp, embedded = false }: IntelligencePanelProps) {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const skipPropertyFetch = embedded && propertyNameProp
        const fetches: Promise<Response>[] = [
          fetch(`/api/properties/${propertyId}/intelligence`, { credentials: 'include' }),
        ]
        if (!skipPropertyFetch) {
          fetches.push(fetch(`/api/properties/${propertyId}`, { credentials: 'include' }))
        }
        const results = await Promise.all(fetches)
        const intelRes = results[0]
        if (intelRes.ok) {
        ...
        if (!skipPropertyFetch && results[1]) {
    ...
    fetchData()
  }, [propertyId, embedded, propertyNameProp])
  ...
  const doSave = useCallback(async (patch: Record<string, any>) => {
    ...
    setSaving(true)
    try {
      const res = await fetch(`/api/properties/${propertyId}/intelligence`, {
        method: 'PATCH',
  ...
  if (loading) {
    return (
      <div className={embedded ? "flex items-center justify-center py-12" : "min-h-screen flex items-center justify-center bg-gray-50"}>
        <motion.div
          animate={{ rotate: 360 }}
  ...
  // ---- Shared content (progress bar, info banner, questions, sections, sticky save bar) ----
  const sharedContent = (
    <>
      {/* Progress bar */}
  ...
  // ---- Sticky save bar (shared between both modes) ----
  const stickySaveBar = (
    <AnimatePresence>
  ...
  // ---- Embedded mode: no page wrapper, no header, no back button ----
  if (embedded) {
    return (
      <>
        {sharedContent}
  ...
  // ---- Full page mode (default): identical to original page ----
  return (
